...
USER_SESSION_ID = file.read()       # Get your session by inspecting the session cookie content in your web browser while connected to adventofcode and paste it here as plain text in between the ". Leave at is to not download inputs.
...
author = "Mathai Mathew"            # Name automatically put in the code templates.
...
dates = "December 2022"              # Date automatically put in the code templates.
starting_advent_of_code_year = 2022 # You can go as early as 2015.
last_advent_of_code_year = 2022     # The setup will download all advent of code data up until that dates included
last_advent_of_code_day = int(date.today().strftime("%d"))    # If the year isn't finished, the setup will download days up until that day included for the last year
# Imports
import os
try:
    import requests
